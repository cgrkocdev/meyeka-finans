...
export function Dashboard(){
 ...
 const categories=useMemo(()=>Object.entries(filteredExpense.reduce<Record<string,number>>((acc,row)=>{const key=row.category??row.detail??"Diğer";acc[key]=(acc[key]??0)+Number(row.amount);return acc},{})).map(([name,value],index)=>({name,value,color:colors[index%colors.length]})),[filteredExpense]);
