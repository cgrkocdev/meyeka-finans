...
async function addRecord(path,button,name,amount){await open(path);await page.getByRole("button",{name:button}).click();const form=page.locator("form").last();assert.equal(await form.evaluate(f=>f.checkValidity()),false,"Boş form geçersiz olmalı");await form.locator('[name="name"]').fill(name);await form.locator('[name="amount"]').fill(String(amount));await form.locator('[name="category"]').selectOption({index:1});await form.locator('[name="description"]').fill("E2E test açıklaması");await form.getByRole("button",{name:"Kaydet"}).click();await page.getByText(name,{exact:true}).waitFor();const search=page.getByPlaceholder("İşlem, kategori veya açıklama ara...");await search.fill(name);assert.equal(await page.getByText(name,{exact:true}).count(),1);await search.fill("");const download=page.waitForEvent("download");await page.getByRole("button",{name:"Dışa aktar"}).click();assert.ok((await download).suggestedFilename().endsWith(".csv"));await page.getByTitle("Sil").first().click();assert.equal(await page.getByText(name,{exact:true}).count(),0)}
